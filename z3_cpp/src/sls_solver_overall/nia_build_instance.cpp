...
namespace nia_overall
{
    ...
    void ls_solver::set_components(std::vector<double> &x_pos, std::vector<double> &y_pos, std::vector<double> &widths, std::vector<double> &hights, std::vector<bool> &visibles, std::vector<std::string> &c_names)
    {
        ...
        y_pos.resize(component_num);
        widths.resize(component_num);
        hights.resize(component_num);
        visibles.resize(component_num);
        c_names = component_names;
    }
    ...
    void ls_solver::find_unit_equal_lits(std::vector<int> &unit_equal_lits_all)
    {
        ...
        std::vector<int> unit_inequal;
        unit_inequal.reserve(_num_lits);
        ...
        for (int clause_idx = 0; clause_idx < _clauses.size(); clause_idx++)
        {
            if (_clauses[clause_idx].literals.size() == 1 && _clauses[clause_idx].literals[0] > 0)
            {
                ...
                { // t1+t2<=0
                    ...
                    unit_inequal.push_back(lit_idx);
                }
            }
        }
        for (size_t pre_idx = 0; pre_idx < unit_inequal.size(); pre_idx++)
        {
            lit *l_pre = &(_lits[unit_inequal[pre_idx]]);
            if (l_pre->lits_index == 0)
                continue;
            for (size_t pos_idx = pre_idx + 1; pos_idx < unit_inequal.size(); pos_idx++)
            {
                lit *l_pos = &(_lits[unit_inequal[pos_idx]]);
                if (l_pos->lits_index == 0)
                    continue;
                if (is_neg_lit(*l_pre, *l_pos))
                {
                    l_pos->lits_index = 0;
                    l_pre->is_equal = true;
                    unit_equal_lits_all.push_back(l_pre->lits_index);
                }
            }
        }
    }
    void ls_solver::equal_vars(ration_num _bc_width)
    {
        ...
        { // if the formula has been modified, it should try to update by equalities again
            ...
            if (update_by_equals(unit_equal_lits_all))
                modified = true;
        }
    }
    ...
    bool ls_solver::update_by_equals(const std::vector<int> &unit_equal_lits_all)
    {
    ...
    void ls_solver::build_instance_original(const std::vector<std::string> &unit_lits)
    {
        ...
        equal_vars();
        reduce_clause();
        record_info_after_build_origin();
    }

    // record the lit/var/clause information after build_origin
    void ls_solver::record_info_after_build_origin()
    {
        _clauses_after_origin = _clauses;
        _lits_after_origin = _lits;
        _vars_after_origin = _vars;
        fa_after_origin = fa;
        fa_coff_after_origin = fa_coff;
        fa_const_after_origin = fa_const;
        preset_values_after_origin = preset_values;
    }

    void ls_solver::restore_info_before_build_new_width()
    {
        _clauses = _clauses_after_origin;
        _lits = _lits_after_origin;
        _vars = _vars_after_origin;
        fa = fa_after_origin;
        fa_coff = fa_coff_after_origin;
        fa_const = fa_const_after_origin;
        preset_values = preset_values_after_origin;
    ...
    void ls_solver::prepare_components_idx()
    {
        ...
        for (const auto &nia_v_idx : nia_var_vec)
        {
            ...
            if ((v_end == "x" || v_end == "y" || v_end == "width" || v_end == "hight") && (v_name != basic_component_name))
            {
    ...
    void ls_solver::prepare_soft_components_idx(const std::vector<std::string> &soft_c_names)
    {
        ...
        {
            soft_components_idx.push_back(name2var[soft_c_name + "_width"]);
            soft_components_idx.push_back(name2var[soft_c_name + "_hight"]);
            soft_components_idx.push_back(name2var[soft_c_name + "_x"]);
            soft_components_idx.push_back(name2var[soft_c_name + "_y"]);
