...
namespace nia_overall
{
    ...
    public:
        ...
        void build_instance_new_width(int _bc_width);
        void find_bound();
        ...
        void equal_vars(ration_num _bc_width = 0); // find out the equalilty among vars, and reset the vars and lits
        int bc_width_idx = 0;
        void eliminate_multiple_inequalities(); // a+b<=3 a+b<=2 -->a+b<=2
        bool resolve_multiple_equals(const std::vector<int> &m_lits);
        bool update_by_equals(const std::vector<int> &unit_equal_lits_all);
        ...
        void record_info_after_build_origin();
        void restore_info_before_build_new_width();
        std::vector<variable> _vars_after_origin;
        std::vector<clause> _clauses_after_origin;
        std::vector<lit> _lits_after_origin;
        std::vector<int> fa_after_origin;
        std::vector<ration_num> fa_coff_after_origin;
        std::vector<ration_num> fa_const_after_origin;
        std::vector<ration_num> preset_values_after_origin;
