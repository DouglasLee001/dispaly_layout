...
namespace nia
{
    ...
    bool ls_solver::update_width_hight(ration_num new_pre_val_width, ration_num new_pre_val_hight)
    {
        ...
        new_pre_val_width = (new_pre_val_width - fa_const_vars[b_width_idx]) / fa_coff_vars[b_width_idx];
        new_pre_val_hight = (new_pre_val_hight - fa_const_vars[b_hight_idx]) / fa_coff_vars[b_hight_idx];
        return (update_preset_value(fa_vars[b_hight_idx], new_pre_val_hight) &&
                update_preset_value(fa_vars[b_width_idx], new_pre_val_width));
    }
}
