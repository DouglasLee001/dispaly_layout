...
namespace nia
{
    ...
    void ls_solver::print_components(int offset_x, int offset_y)
    {
        ...
        for (size_t c_idx = 0; c_idx < component_num - 1; c_idx++)
        {
            if (_solution[components_idx[5 * c_idx + 4]] < 0)
                continue;
            std::cout << "{ name: \'" << component_names[c_idx] << "'";
            std::cout << ", width: " << _solution[components_idx[5 * c_idx]].print();
            std::cout << ", height: " << _solution[components_idx[5 * c_idx + 1]].print();
            std::cout << ", x: " << (_solution[components_idx[5 * c_idx + 2]] + offset_x).print();
            std::cout << ", y: " << (_solution[components_idx[5 * c_idx + 3]] + offset_y).print();
    ...
    void ls_solver::print_full_model()
    {
        ...
        print_components();
        size_t c_idx = component_names.size() - 1;
        std::cout << "{ name: \'" << basic_component_name;
        std::cout << "', width: " << _solution[components_idx[5 * c_idx]].print();
        std::cout << ", height: " << _solution[components_idx[5 * c_idx + 1]].print();
        std::cout << ", x: 0, y: 0, display: \'block\' },\n";
        print_end();
    }
    ...
    void ls_solver::up_bool_vars()
    {
        for (size_t v_idx = 0; v_idx < _num_vars; v_idx++)
        {
            if (!var_appear[v_idx])
            {
                ...
                {
                    int root_idx = fa_vars[v_idx];
                    if (root_idx != v_idx)
                        _solution[v_idx] = _solution[root_idx] * fa_coff_vars[v_idx] + fa_const_vars[v_idx];
                }
