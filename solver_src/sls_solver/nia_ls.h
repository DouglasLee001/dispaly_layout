...
namespace nia
{
    ...
    public:
        ...
        // fa[x]=f means x=(coff*f)+const these are dedicated for resolution vars
        std::vector<int> fa;
        std::vector<ration_num> fa_coff;
        std::vector<ration_num> fa_const;
        // this is dedicated for vars
        std::vector<int> fa_vars;
        std::vector<ration_num> fa_coff_vars;
        std::vector<ration_num> fa_const_vars;
        ...
        void equal_vars();                      // find out the equalilty among vars, and reset the vars and lits
        void eliminate_multiple_inequalities(); // a+b<=3 a+b<=2 -->a+b<=2
        bool resolve_multiple_equals(const std::vector<int> &m_lits);
        bool update_by_equals(const std::vector<int> &unit_equal_lits_all, std::vector<ration_num> &preset_values);
        ...
        void update_lit_preset_tmp_var(int lit_idx, const std::vector<ration_num> &pre_set_val);
        uint64_t transfer_name_to_reduced_var(const std::string &name, bool is_nia);    // after the resolution vars are inserted into _vars
        uint64_t transfer_name_to_resolution_var(const std::string &name, bool is_nia); // bool var is inserted into _resolution_var when build lit
        uint64_t transfer_name_to_tmp_var(const std::string &name);                     // nia var is first inserted into _tmp_var when build lit, then inserted into _resolution_var when reduce var(x-y->z)
        void turn_tmp_to_resolution();                                                  // turn tmp vars to resolution vars
        void turn_resolution_to_reduce();                                               // turn resolution to vars
        ...
        bool reduce_duplicated_lits(); // return true if found duplicated lits
        ...
        // construction
        void construct_solution(ls_solver *other_solver = nullptr);
